...
const SocialRow = ({ data }: { data: SOCIALS }) => {
  return (
    <tr className={styles.row}>
      ...
      <td>{data.status ? <span className={styles.joined}>
        {
          data.account === SOCIAL_PROVIDERS.Discord ?
            'Joined Kyzzen\'s Discord server' :
            data.account === SOCIAL_PROVIDERS.X ?
              'Following Kyzzen' :
              data.account === SOCIAL_PROVIDERS.Mail ?
                'Suscribed to Kyzzen Newsletter' :
                data.account === SOCIAL_PROVIDERS.Telegram ?
